...
% for i = 1:length(dt)
    ...
    [coe1_e, r1_e, v1_e, jd1_e] = planet_elements_and_sv(3, 2022, 01, 01, 18, 00, 00);
    
    
    % Position of Jupiter at the arrival  (km)     
    [coe2_v, r2_v, v2_v, jd2_v] = planet_elements_and_sv(2, 2022, 04, 01, 18, 00, 00);
    
    string = 'pro';
    %...
    %dt = year2seconds(3)+ month2seconds(2)+days2seconds(9);     % Total TOF (speriamo)
    %...Algorithm 5.2:
    [v1_l_e, v2_l_v] = lambert(r1_e, r2_v, dt, string);
    v2_l_v
    norm(v2_l_v)
    ...
    coe = coe_from_sv(r2_v, v2_l_v, mu);
    % Final true anomaly:
    TA2 = rad2deg(coe(6));
...
[~, r2_s, v2_s, ~] = planet_elements_and_sv(6, 2028, 04, 01, 00, 00, 00);
...
a_flyby_Venus = - mu_Venus/((v_inf_down_norm_Venus)^2);%semiaxis major 
r_p_flyby_Venus = 7e3:2e3:5e5 ;  %hp  
%r_p_flyby_Jupiter = 1e5;
for j = 1:length(r_p_flyby_Venus)
e_flyby_Venus = 1-(r_p_flyby_Venus/a_flyby_Venus); 

delta_Venus = 2*asin(1/e_flyby_Venus(j)); %angolo tra gli asintoti 
...
mu  = 1.327*10^11;
coe_flyby = coe_from_sv(r2_fin_v,v_fin_Venus,mu);
...
for i = 1:length(Ta_for_lambert)
    ...
    if d_V_norm < 11 && e_flyby_Venus(j)< 10 && dT < month2seconds(6)
